<?php $this->view('Partials/head', ['pageTitle' => $pageTitle ?? 'Projets a valider']); ?>
<body>
                ...
                <div class="card common-card mb-4 border-0" style="background: transparent; box-shadow: none;">
                    <div class="card-body p-0 d-flex flex-wrap justify-content-between align-items-center gap-3">
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <div cl>
                                <h3 class="mb-0 fw-800 text-primary">Projets à Valider</h3>
                                <p class="text-muted small mb-0">Gestion de la file d'attente administrative</p>
                            </div>
                        </div>
                        <div class="d-flex gap-2">
                             <a href="<?= ROOT ?>/Admins/projects_management" class="btn btn-secondary btn-sm rounded-pill px-4">📂 Tous les projets</a>
                        </div>
                    </div>
                </div>
                ...
                <div class="card common-card">
                    <div class="card-body p-4">
                        ...
                        <div class="filter-shelf">
                            <form method="GET" action="<?= ROOT ?>/Admins/pending_projects" class="row gy-3">
                                <div class="col-md-4">
                                    <label class="small fw-bold text-muted mb-2">Recherche</label>
                                    <input type="text" name="search" value="<?= htmlspecialchars($projectSearch) ?>" class="form-control rounded-3 border-0 bg-white" placeholder="Titre, auteur ou catégorie...">
                                </div>
                                <div class="col-md-3">
                        ...
                        <form method="POST" action="<?= ROOT ?>/Admins/pending_projects">
                            ...
                            <div class="admin-bulk-bar shadow-sm">
                                <div class="form-check mb-0">
                                    <input class="form-check-input" type="checkbox" id="select-all-pending" style="width: 1.2rem; height: 1.2rem;">
                                    <label class="form-check-label fw-bold small ms-2" for="select-all-pending">TOUT SÉLECTIONNER</label>
                                </div>
                                <div class="admin-bulk-actions">
                                    <button class="btn btn-success btn-sm px-4" type="submit" name="bulk_validate_projects">✅ Tout Valider</button>
                                    <button class="btn btn-danger btn-sm px-4" type="submit" name="bulk_reject_projects" onclick="return confirm('Rejeter les projets selectionnes ?');">❌ Tout Rejeter</button>
                                </div>
                            </div>
                                ...
                                <tbody>
                                    <?php if (!empty($projects)): ?>
                                        <?php foreach ($projects as $project): ?>
                                            ...
                                            ?>
                                            <tr>
                                                <td><input type="checkbox" class="pending-project-checkbox form-check-input" name="project_ids[]" value="<?= (int)($project->id ?? 0) ?>" style="width: 1.1rem; height: 1.1rem;"></td>
                                                ...
                                                <td class="admin-table-actions">
                                                    <div class="d-flex gap-2">
                                                        <a href="<?= ROOT ?>/Admins/project_detail/<?= (int)($project->id ?? 0) ?>" class="btn btn-light btn-sm border" title="Voir détails">👁️</a>
                                        ...
                                        <?php endforeach; ?>
                                    <?php else: ?>
                                        <tr><td colspan="7" class="text-center text-muted">Aucun projet en attente.</td></tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                            </div>
                        ...
                        <?php $this->view('Partials/admin-pagination', [
                            ...
                            'basePath' => 'Admins/pending_projects',
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var checkboxes = Array.from(document.querySelectorAll('.pending-project-checkbox'));
